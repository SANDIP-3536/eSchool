<div class="footer">
    <div class="row">
        <div class="col-sm-4">
            <div class="pull-left">
                <strong>Copyright </strong><a href="http://www.syntech.co.in" target="_blank"> <img src="<?=base_url()?>assets/img/syntech_logo.png" style="height:13px;"></a><strong> &copy; 2017-2018 </strong>  
            </div>
        </div>
        <?php if(!empty($school_logo)){  ?>
        <div class="col-sm-4">
            <center>
                <div>
                   <img src="<?php if(!empty($school_logo)){echo $school_logo;} ?>" style="height:20px;width:20px;"> <strong style="color:#ffffff;"><?php if(!empty($school_name)){echo $school_name;} ?> </strong> 
                </div>
            </center>
        </div>
        <?php } ?>
        <div class="pull-right">
            <i class="fa fa-phone-square" aria-hidden="true"></i><strong> 020-24269021 / 7030578612</strong> | <i class="fa fa-envelope" aria-hidden="true"></i> <strong>contact@syntech.co.in </strong> 
        </div>
    </div>
</div>



    <!-- Mainly scripts -->
    <!--<script src="<?=base_url()?>assets/js/jquery-2.1.1.js"></script>-->
    <script src="https://code.jquery.com/jquery-2.2.4.js"></script>
    <script src="<?= base_url();?>assets/js/plugins/dataTables/datatables.min.js"></script>
    <script src="https://cdn.datatables.net/buttons/1.5.2/js/dataTables.buttons.min.js"></script>
    <script src="https://cdn.datatables.net/buttons/1.5.2/js/buttons.colVis.min.js"></script>
    <script src="<?=base_url()?>assets/js/bootstrap.min.js"></script>
    <script src="<?=base_url();?>assets/js/jquery-ui.min.js"></script>
    <script src="<?=base_url();?>assets/js/bootstrap-datepicker.js"></script>
    <script src="<?=base_url()?>assets/js/plugins/metisMenu/jquery.metisMenu.js"></script>
    <script src="<?=base_url()?>assets/js/plugins/slimscroll/jquery.slimscroll.min.js"></script>
    <script src="<?=base_url();?>assets/js/plugins/select2/select2.full.min.js"></script>

    <!-- Custom and plugin javascript -->
    <script src="<?=base_url()?>assets/js/inspinia.js"></script>
    <script src="<?=base_url()?>assets/js/plugins/pace/pace.min.js"></script>

    <!-- Date range use moment.js same as full calendar plugin -->
    <script src="<?=base_url();?>assets/js/plugins/fullcalendar/moment.min.js"></script>

     <!-- Date range picker -->
    <script src="<?=base_url();?>assets/js/plugins/daterangepicker/daterangepicker.js"></script>

    <script src="<?=base_url()?>assets/js/plugins/sweetalert/sweetalert.min.js"></script>

    <!-- clockpicker -->
     <script src="<?=base_url()?>assets/js/plugins/clockpicker/clockpicker.js"></script>
    
    <script src="<?= base_url();?>assets/js/plugins/validate/jquery.validate.min.js"></script>
    <script src="<?= base_url();?>assets/js/plugins/validate/additional-methods.min.js"></script>
    <script type="text/javascript">
        $(window).load(function() {
            $(".loader").fadeOut("slow");
        });
    </script>
    <script>
        
        $.validator.setDefaults({
            submitHandler: function (form) {    
                form.submit();
            }
        });

        <?php if($report == 'student'){?>
             $('#student').addClass('active');
             document.title = "eSchool | Student Report"
        <?php } ?>

        $(document).on('change','#general_class_report_details,#class_class_report_details,#division_class_report_details,#gender_class_report_details,#cast_class_report_details,#town_class_report_details,#contact_class_report_details',function(){
            $('#general_class_name_report_details').empty();
            $('.general_division_report_details,#class_division_report_details,#division_division_report_details,#gender_division_report_details,#cast_division_report_details,#town_division_report_details').empty();
            var class_id = $(this).val();
            $.post('<?=site_url('Reports/fetch_class_division')?>',{class_id:class_id},function(data){
                // console.log(data);
                $('.general_division_report_details,.class_division_report_details,#division_division_report_details,#gender_division_report_details,#cast_division_report_details,#town_division_report_details').append("<option value='0'>All Division</option>");
                $.each(data,function(k,v){
                    $('.general_division_report_details,.class_division_report_details,#division_division_report_details,#gender_division_report_details,#cast_division_report_details,#town_division_report_details').append("<option value="+v.division_id+">"+v.division_name+"</option>")
                });
            },'json');

            $.post('<?=site_url('Reports/fetch_class_name')?>',{class_id:class_id},function(data){
                console.log(data);
                $.each(data,function(k,v){
                    $('#general_class_name_report_details').val(v.class_name)
                });
            },'json');
        });

        $(document).on('change','.general_division_report_details,.class_division_report_details,#division_division_report_details,#gender_division_report_details,#cast_division_report_details,#town_division_report_details',function(){
            $('#general_division_name_report_details').empty();
            var division = $(this).val();
            $.post('<?=site_url('Reports/fetch_class_division_name')?>',{division:division},function(data){
                console.log(data);
                $.each(data,function(k,v){
                    $('#general_division_name_report_details').val(v.division_name)
                });
            },'json');
        });

        var pdfImg = '';
        const toDataURL = url => fetch(url)
          .then(response => response.blob())
          .then(blob => new Promise((resolve, reject) => {
            const reader = new FileReader()
            reader.onloadend = () => resolve(reader.result)
            reader.onerror = reject
            reader.readAsDataURL(blob)
        }));

        toDataURL('<?php echo $school_report_header ?>')
        .then(dataUrl => {
            pdfImg = ''+dataUrl+'';
        });

// -------------------------------------------------------------------------- General Register Reports ------------------------------------------------------------
        $('#general_division_report,#class_division_report').hide();
        $(document).on('change','#general_class_report_details',function(){
            var class_id = $(this).val();
            if(class_id != 0){
                $('#general_division_report').show(700);
                $('#class_division_report').show(700);
            }
            else{
                $('#general_division_report').hide(700);
                $('#class_division_report').hide(700);
            }
        });

         var genral = $('.dataTables-example').DataTable({
            pageLength: 10,
            responsive: true,
            ordering: false,
            // sScrollX: true,
            // sScrollXInner: true,
            dom: '<"html5buttons"B>lTfgitp',
            buttons: [
                {
                    "extend": 'print',
                   
                    title:'',
                    // footer:true,
                    customize: function ( win ) {
                      $(win.document.body).find( 'thead' ).prepend('<tr><td colspan="10" style="font-size:24px;text-align:center;border:none !important;"><img src="<? echo $school_report_header;?>" width="100%"> </td></center></tr>')
                    },
                    exportOptions: {
                        columns: ':visible'
                    }
                },
                {"extend": 'excel', title: 'Genral Register'},
                // {
                //     "extend": 'pdf',
                //     // "footer": true, 
                //     title: 'General Register',
                //     // exportOptions: {
                //     //     columns: ':visible'
                //     // },
                //     pageSize: 'A4',
                //     sWidth: '1200px',
                //     customize : function(doc) {
                //         doc.content[2].widths = [ '100%', '12%', '12%', '18%', '28%', '10%', '10%'];
                //     },
                //     customize: function ( doc ) {
                //         // Splice the image in after the header, but before the table
                //         doc.content.splice( 0, 0, {
                //             margin: 0,
                //             alignment: 'center',
                //             image: ''+pdfImg+'',
                //             width: 520,
                //             height: 50, 
                //         });
                //         doc.content[2].margin = [ 0, 0, 0,0  ]
                //     }
                // },
            ],
            "columnDefs": [
                {
                    "targets": [ 15 ],
                    "visible": true
                }
            ],
            language: {
                "emptyTable": "<img src='http://eschool.syntech.co.in/eschool/assets/img/No-record-found.png'> "
            }
        });

        genral.columns( [11,12,13,14,15,16,17,18] ).visible(false);

        $('a.toggle-vis1').on( 'click', function (e) {
            e.preventDefault();
     
            // Get the column API object
            var column = genral.column( $(this).attr('data-column') );     
            // Toggle the visibility
            column.visible( ! column.visible() );
            if($(this).hasClass("btn-default")){
                $(this).removeClass();
                $(this).addClass("toggle-vis btn btn-sm btn-primary");
            }
            else{
                $(this).removeClass();
                $(this).addClass("toggle-vis btn btn-sm btn-default");
            }
        });

        $(".loader_half").fadeOut("slow");

// -------------------------------------------------------------------Government Report -------------------------------------------------------
       

    $(document).on('click','.govern_class_division_report',function(){
        $(".loader_half").fadeIn("slow");

        if (!$.fn.DataTable.isDataTable('.dataTables-example-gov'))
        {
            $.post('<?=site_url('Reports/government_report')?>',{},function(data){
                // console.log("data"+data);
                var government_data = data.government;
                // console.log("fee_due_report"+due_fee_report_data);
                var data_gov = $('.dataTables-example-gov').DataTable({
                    pageLength: 10,
                    responsive: false,
                    ordering: false,
                    // sScrollX: true,
                    // sScrollXInner: true,
                    data: government_data,
                    columns:[
                        {title: "GRN No.",data:"student_GRN"},
                        {title: "A.Y.",data:"AY_name"},
                        {title: "Class",data:"class_name"},
                        {title: "Division",data:"division_name"},
                        {title: "Student L Name",data:"student_last_name"},
                        {title: "Student F Name",data:"student_first_name"},
                        {title: "Student M Name",data:"student_middle_name"},
                        {title: "DOB",data:"DOB"},
                        {title: "Gender",data:"student_gender"},
                        {title: "Mother L Name",data:"parent_last_name"},
                        {title: "Mother F Name",data:"parent_first_name"},
                        {title: "Mother M Name",data:"parent_middle_name"},
                        {title: "UDISE No",data:null,defaultContent:"27350504907"},
                        {title: "Semi English",data:null,defaultContent:"No"},
                        {title: "Admission Date",data:"admission_date"},
                        {title: "Admission Class",data:"class_of_admission"},
                        {title: "Admission Type",data:null},
                        {title: "Religion",data:"student_religion"},
                        {title: "Category",data:"student_category"},
                        {title: "BPL",data:null,defaultContent:"No"},
                        
                    ],
                    dom: '<"html5buttons"B>lTfgitp',
                    "buttons": [
                        // {extend: 'copy'},
                        // {
                        //     "extend": 'print',
                           
                        //     title:'',
                        //     // footer:true,
                        //     customize: function ( win ) {
                        //       $(win.document.body).find( 'thead' ).prepend('<tr><td colspan="12" style="font-size:24px;text-align:center;border:none !important;">'+class_name+' Student Parent Contact Details </td></center></tr>')
                        //     },
                        // },
                        {"extend": 'excel', title: 'All Student Details'},
                        // {"extend": 'pdf', title: ''+class_name+' Student Parent Contact Details'},
                    ],
                    "language": {
                        "emptyTable": "<img src='<?=base_url();?>assets/img/No-record-found.png'> "
                    }
                });

                if (data != "-1") 
                { 
                    $(".loader_half").fadeOut("slow");
                }
             
            },'json');

        }else{

            $(".loader_half").fadeOut("slow");
        }
        
    });


// -------------------------------------------------------------------------------- All Generic Report -----------------------------------------------------
    var columns1 = [];
    $(document).on('click','.All_report',function(){
        $(".loader_half").fadeIn("slow");

        if (!$.fn.DataTable.isDataTable('.dataTables-example-all'))
        {
            $.post('<?=site_url('Reports/all_report')?>',{},function(data){
                // console.log("data"+data);
                var all_report_data = data.all_report;
                // console.log("fee_due_report"+due_fee_report_data);
                var all_report = $('.dataTables-example-all').DataTable({
                    pageLength: 10,
                    responsive: true,
                    data: all_report_data,
                    columns:[
                        {title: "Roll No.",data:"SCD_roll_no"},
                        {title: "Student Name",data:"student_name"},
                        {title: "GRN No.",data:"student_GRN"},
                        {title: "Gender",data:"student_gender"},
                        {title: "Adhar Card",data:"student_adhar_card_number"},
                        {title: "DOB",data:"DOB"},
                        {title: "Birth Place",data:"student_birth_place"},
                        {title: "Blood Group ",data:"student_blood_group"},
                        {title: "Town",data:"student_present_town"},
                        {title: "Address",data:"student_address"},
                        {title: "Admission Date",data:"admission_date"},
                        {title: "Class",data:"class_name"},
                        {title: "Division",data:"division_name"},
                        {title: "Category",data:"student_category"},
                        {title: "Religion",data:"student_religion"},
                        {title: "Cast",data:"student_cast"},
                        {title: "Sub Cast",data:"student_sub_cast"},
                        {title: "Father Name",data:"father_name"},
                        {title: "Mobile No.",data:"father_mobile_number"},
                        {title: "Mother Name",data:"mother_name"},
                        {title: "Mobile No.",data:"mother_mobile_number"},
                        {title: "Signature",data:null,defaultContent:""},
                        
                    ],
                    dom: '<"html5buttons"B>lTfgitp',
                    "buttons": [
                        {
                            "extend": 'print',
                           
                            title:'',
                            // footer:true,
                            customize: function ( win ) {
                              $(win.document.body).find( 'thead' ).prepend('<tr><td colspan="12" style="font-size:24px;text-align:center;border:none !important;"><img src="<? echo $school_report_header;?>" width="100%"> </td></center></tr>')
                            },
                            exportOptions: {
                                columns: ':visible'
                            }
                        },
                        {
                            extend: 'pdfHtml5',
                            exportOptions: {
                                columns: ':visible'
                            },
                            //customize: function ( doc ) {
                                //doc.content.splice( 0, 0, {
                                    //margin: [ 0, 0, 0, 0 ],
                                    //alignment: 'center',
                                    //image: 'data:image/png;base64,'
                        //         } );
                        //     }
                        },
                        {
                            "extend": 'excel', title: 'All Student Details',
                            messageTop:'<img src="<? echo $school_report_header;?>" width="100%">',
                            exportOptions: {
                                columns: ':visible'
                            }
                        },
                    ],
                    "language": {
                        "emptyTable": "<img src='<?=base_url();?>assets/img/No-record-found.png'> "
                    },
                });
                $('.dataTables-example-all tfoot th').each( function () {
                        var title = $(this).text();
                        $(this).html( '<input type="text" style="border: none;" placeholder="'+title+'" />' );
                } );
                var all_report = $('.dataTables-example-all').DataTable();
             
                // Apply the search
                all_report.columns().every( function () {
                    var that = this;
                    $( 'input', this.footer() ).on( 'keyup change', function () {
                        if ( that.search() !== this.value ) {
                            that
                                .search( this.value )
                                .draw();
                        }
                    } );
                } );

                all_report.columns( [4,5,6,7,8,9,10,13,14,15,16,17,18,19,20,21] ).visible( false );


        // genral.row.child();

                $('a.toggle-vis').on( 'click', function (e) {
                    e.preventDefault();
             
                    // Get the column API object
                    var column = all_report.column( $(this).attr('data-column') );     
                    // Toggle the visibility
                    column.visible( ! column.visible() );
                    if($(this).hasClass("btn-default")){
                        $(this).removeClass();
                        $(this).addClass("toggle-vis btn btn-sm btn-primary");
                        // columns1.push($(this).title);
                        // alert(columns1);
                    }
                    else{
                        $(this).removeClass();
                        $(this).addClass("toggle-vis btn btn-sm btn-default");
                    }
                });

        // $( all_report.table().container() ).on( 'keyup', 'tfoot input', function () {
        //     all_report
        //         .column( $(this).data('index') )
        //         .search( this.value )
        //         .draw();
        // });

                if (data != "-1") 
                { 
                    $(".loader_half").fadeOut("slow");
                }

            },'json');

        }else{

            $(".loader_half").fadeOut("slow");
        
        }
    });

// --------------------------------------------------------------------- STudent SMS Report -------------------------------------------------------------------
    $(document).on('click','.sms_report',function(){
        $(".loader_half").fadeIn("slow");

        if (!$.fn.DataTable.isDataTable('.dataTables-example-SMS'))
        {
            $.post('<?=site_url('Reports/sms_report')?>',{},function(data){
                console.log(data);
                var total_sms_report_data = data.total_sms_report;

                var sms = $('.dataTables-example-SMS').DataTable({
                    pageLength: 10,
                    responsive: true,
                    ordering: false,
                    // sScrollX: true,
                    // sScrollXInner: true,
                    data: total_sms_report_data,
                    columns:[
                        {title: "Date",data:"sms_date"},
                        {title: "Time",data:"sms_time"},
                        {title: "Std/Emp",data:"type"},
                        {title: "SMS Type",data:"sms_type"},
                        {title: "Mobile No.",data:"sent_sms_mobile_number"},
                        {title: "Name",data:"name"},
                        {title: "MSG ID",data:"sent_sms_MsgID"},
                        {title: "Count",data:"sent_sms_count"},
                        {title: "Message",data:"sent_sms_MSG"},
                        
                    ],
                    dom: '<"html5buttons"B>lTfgitp',
                    "buttons": [
                        {"extend": 'excel', title: 'All Student SMS Details'},
                    ],
                    "language": {
                        "emptyTable": "<img src='<?=base_url();?>assets/img/No-record-found.png'> "
                    },

                });

            // sms.on( 'draw', function () {
            //     console.log("complete");
            // } );
                if (data != "-1") 
                { 
                    $(".loader_half").fadeOut("slow");
                }

          },'json');

        }else{

            $(".loader_half").fadeOut("slow");
        
        }
        

    });

// -------------------------------------------------------------------Government Report -------------------------------------------------------
    $(document).on('click','.doc_report',function(){
        $(".loader_half").fadeIn("slow");

        if (!$.fn.DataTable.isDataTable('.dataTables-example-doc'))
        {
            $.post('<?=site_url('Reports/doc_report')?>',{},function(data){
                // console.log("data"+data);
                var doc_report_data = data.doc_report;

                var doc = $('.dataTables-example-doc').DataTable({
                    pageLength: 10,
                    responsive: true,
                    ordering: false,
                    data: doc_report_data,
                    columns:[
                        {title: "GRN No.",data:"student_GRN"},
                        {title: "Student Name",data:"student_name"},
                        {title: "Class",data:"class_name"},
                        {title: "Division",data:"division_name"},
                        {title: "Student Photo.",data:"student_photo_status"},
                        {title: "Father Photo.",data:"father_photo_status"},
                        {title: "Mother Photo.",data:"mother_photo_status"},
                        {title: "Adhar Card.",data:"aadhar_card_status"},
                        {title: "Transfer Cert.",data:"transfer_certificate_status"},
                        {title: "Leaving Cert.",data:"leaving_certificate_status"},
                        {title: "Birth Cert.",data:"birth_certificate_status"},
                        {title: "Medical Cert.",data:"medicle_certificate_status"},
                    ],
                    dom: '<"html5buttons"B>lTfgitp',
                    "buttons": [
                        // {extend: 'copy'},
                        // {
                        //     "extend": 'print',
                           
                        //     title:'',
                        //     // footer:true,
                        //     customize: function ( win ) {
                        //       $(win.document.body).find( 'thead' ).prepend('<tr><td colspan="12" style="font-size:24px;text-align:center;border:none !important;">'+class_name+' Student Parent Contact Details </td></center></tr>')
                        //     },
                        // },
                        {"extend": 'excel', title: 'Student Document Details',text:'<i class="fa fa-file-excel-o"></i>', titleAttr: 'Excel'},
                        // {"extend": 'pdf', title: ''+class_name+' Student Parent Contact Details'},
                    ],
                    "language": {
                        "emptyTable": "<img src='<?=base_url();?>assets/img/No-record-found.png'> "
                    }

                });

                $('.dataTables-example-doc tfoot th').each( function () {
                        var title = $(this).text();
                        $(this).html( '<input type="text" style="border: none;width: 100%;" placeholder="'+title+'" />' );
                } );
                var doc = $('.dataTables-example-doc').DataTable();
             
                // Apply the search
                doc.columns().every( function () {
                    var that = this;
                    $( 'input', this.footer() ).on( 'keyup change', function () {
                        if ( that.search() !== this.value ) {
                            that
                                .search( this.value )
                                .draw();
                        }
                    } );
                } );
                if (data != "-1") 
                { 
                    $(".loader_half").fadeOut("slow");
                }

            },'json');

        }else{

            $(".loader_half").fadeOut("slow");
        
        }
        
        // $(".loader_half").fadeOut("slow");
    });




        $(".select2_demo").select2({
            placeholder: "Select a Town",
            allowClear: true,
        });  
        $(".select2-container").css("width", "100%"); 
    </script>
</body>


<!-- Mirrored from webapplayers.com/inspinia_admin-v2.6/dashboard_4.html by HTTrack Website Copier/3.x [XR&CO'2014], Sun, 04 Sep 2016 12:30:16 GMT -->
</html>